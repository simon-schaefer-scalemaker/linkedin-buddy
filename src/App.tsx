...
// Cutter View (External)
import { CutterView } from '@/pages/cutter/CutterView'

// Routes that don't use AppLayout or PasswordGate
const EXTERNAL_ROUTES = ['/cutter']

function AppContent() {
  const location = useLocation()
  const isExternalRoute = EXTERNAL_ROUTES.some(route => location.pathname.startsWith(route))
  
  // External routes (like /cutter) don't need the main password gate
  if (isExternalRoute) {
    return (
      <Routes>
  ...
  // All other routes are protected by PasswordGate
  return (
    <PasswordGate>
      <AppLayout>
        <Routes>
